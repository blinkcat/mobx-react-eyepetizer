...
export default class Detail extends Component {
	...
	render() {
		const {
			videoStore: { videoInfo, isFetching },
			relatedVideoListStore: { list },
			replyListStore: { list: replyList }
		} = this.props;
				...
				<div className={styles.inner}>
					...
					<div className={styles.relative}>
						<div className={styles.relative_inner}>
							<VideoInfo
								{...omit(videoInfo, ['playUrl', 'coverForFeed', 'coverBlurred'])}
							/>
				...
				<Link key={v.id} to={`/detail/${v.id}`}>
					<div className={styles.related_item}>
						<div
							...
							style={{ backgroundImage: `url(${v.coverForFeed})` }}
						/>
						<div className={styles.meta}>
							<div className={styles.title}>{v.title}</div>
							<div className={styles.category}>{`#${v.category} / ${second2minute(
								v.duration
							)}`}</div>
						</div>
					</div>
				...
				{list.map(v => (
					<div className={styles.replyb} key={v.id}>
						...
						<div
							...
							style={{ backgroundImage: `url(${v.user.avatar})` }}
						/>
